...
void Mosquito::initialize(Config *config) {
    ...
    genotypes_table = std::vector<std::vector<std::vector<Genotype *>>>(
            ...
            std::vector<std::vector<Genotype *>>(config->number_of_locations(),
                                                 std::vector<Genotype *>(config->mosquito_config().prmc_size,
                                                                         nullptr)));
    ...
    if (!config->mosquito_config().interrupted_feeding_rate_raster.empty()) {
        ...
        if (config->mosquito_config().interrupted_feeding_rate.size() == 1) {
            ...
            config->mosquito_config().interrupted_feeding_rate = std::vector<double>(config->number_of_locations(),
                                                                                     if_rate);
        } else if (config->mosquito_config().interrupted_feeding_rate.size() != config->number_of_locations()) {
            LOG(FATAL)
                    << "Number of element of interrupted feeding rate should be 1 or equal to number of locations!!!!";
        }
    }
    ...
    for (int loc = 0; loc < config->number_of_locations(); loc++) {
        ...
        for (int if_index = 0; if_index < interrupted_feeding_indices.size(); ++if_index) {
            ...
            if (config->within_host_induced_free_recombination()) {
                ...
                get_genotypes_profile_from_person(first_sampling[if_index], sampling_genotypes,
                ...
                if (interrupted_feeding_indices[if_index]) {
                    ...
                    get_genotypes_profile_from_person(second_sampling[temp_if], sampling_genotypes,
                                                      relative_infectivity_each_pp);
                    Model::DATA_COLLECTOR->mosquito_recombined_genotype_resistant_count()[loc][15]++;
                }
...
//        }
                ...
                auto first_genotype =
                        random->roulette_sampling_tuple<Genotype>(1, relative_infectivity_each_pp, sampling_genotypes,
                                                                  false)[0];
...
//                        printf("\n");
    ...
    //                    printf("[SINGLE2] therapy: %s\n", therapies[therapy_id - 6].c_str());
    //                    printf("[SINGLE2] minEC50 ART: %f\n", drug_id_min_ec50[sc_therapy->drug_ids[0]]);
    //                    printf("[SINGLE2] minEC50 PD: %f\n", drug_id_min_ec50[sc_therapy->drug_ids[1]]);
    //                    printf("[SINGLE2] genotype1 EC50 ART: %f\n", parent_genotypes[0]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0])));
    //                    printf("[SINGLE2] genotype1 EC50 PD: %f\n", parent_genotypes[0]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1])));
    //                    printf("[SINGLE2] genotype2 EC50 ART: %f\n", parent_genotypes[1]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0])));
    //                    printf("[SINGLE2] genotype2 EC50 PD: %f\n", parent_genotypes[1]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1])));
    //                    printf("[SINGLE2] genotype2 EC50 PD: %f\n", parent_genotypes[1]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1])));
                        for(int res_id = 0; res_id < res_23_list.size(); res_id++){
                            if(get_resistant_strength_23(sampled_genotype,res_23_list[res_id])){
    //                            printf("[DOUBLE2] genotype1: %s\n", parent_genotypes[0]->aa_sequence.c_str());
    //                            printf("[DOUBLE2] genotype2: %s\n", parent_genotypes[1]->aa_sequence.c_str());
    //                            printf("[DOUBLE2] recombine: %s\n", sampled_genotype->aa_sequence.c_str());
    //                            printf("[DOUBLE2] %d %d %d %d %d %s\n",
    //                                   Model::SCHEDULER->current_time(),
    //                                   if_index,
    //                                   parent_genotypes[0]->genotype_id,
    //                                   parent_genotypes[1]->genotype_id,
    //                                   sampled_genotype->genotype_id,
    //                                   res_23_list[res_id].c_str());
                                Model::DATA_COLLECTOR->mosquito_recombined_genotype_resistant_count()[loc][5+res_id]++;
                            }
                        }
//                        printf("\n");
                    }
                    if ((((parent_genotypes[0]->get_EC50_power_n(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0])) == drug_id_min_ec50[sc_therapy->drug_ids[0]])
                         ...
                         || ((parent_genotypes[1]->get_EC50_power_n(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0])) == drug_id_min_ec50[sc_therapy->drug_ids[0]])
                             ...
                             && (parent_genotypes[0]->get_EC50_power_n(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1])) == drug_id_min_ec50[sc_therapy->drug_ids[1]])))
                        &&
                        (((parent_genotypes[0]->resist_to(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0])) && !parent_genotypes[0]->resist_to(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1]))
                           && parent_genotypes[1]->resist_to(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1])) && !parent_genotypes[1]->resist_to(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0]))))
                         || (parent_genotypes[1]->resist_to(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0])) && !parent_genotypes[1]->resist_to(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1]))
                             && parent_genotypes[0]->resist_to(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1])) && !parent_genotypes[0]->resist_to(Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0]))))
                             ) {
    //                    printf("[SINGLE3] therapy: %s\n", therapies[therapy_id - 6].c_str());
    //                    printf("[SINGLE3] minEC50 ART: %f\n", drug_id_min_ec50[sc_therapy->drug_ids[0]]);
    //                    printf("[SINGLE3] minEC50 PD: %f\n", drug_id_min_ec50[sc_therapy->drug_ids[1]]);
    //                    printf("[SINGLE3] genotype1 EC50 ART: %f\n", parent_genotypes[0]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0])));
    //                    printf("[SINGLE3] genotype1 EC50 PD: %f\n", parent_genotypes[0]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1])));
    //                    printf("[SINGLE3] genotype2 EC50 ART: %f\n", parent_genotypes[1]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[0])));
    //                    printf("[SINGLE3] genotype2 EC50 PD: %f\n", parent_genotypes[1]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1])));
    //                    printf("[SINGLE3] genotype2 EC50 PD: %f\n", parent_genotypes[1]->get_EC50_power_n(
    //                            Model::CONFIG->drug_db()->at(sc_therapy->drug_ids[1])));
                        for(int res_id = 0; res_id < res_23_list.size(); res_id++){
                            if(get_resistant_strength_23(sampled_genotype,res_23_list[res_id])){
    //                            printf("[DOUBLE3] genotype1: %s\n", parent_genotypes[0]->aa_sequence.c_str());
    //                            printf("[DOUBLE3] genotype2: %s\n", parent_genotypes[1]->aa_sequence.c_str());
    //                            printf("[DOUBLE3] recombine: %s\n", sampled_genotype->aa_sequence.c_str());
    //                            printf("[DOUBLE3] %d %d %d %d %d %s\n",
    //                                   Model::SCHEDULER->current_time(),
    //                                   if_index,
    //                                   parent_genotypes[0]->genotype_id,
    //                                   parent_genotypes[1]->genotype_id,
    //                                   sampled_genotype->genotype_id,
    //                                   res_23_list[res_id].c_str());
                                Model::DATA_COLLECTOR->mosquito_recombined_genotype_resistant_count()[loc][10+res_id]++;
                            }
                        }
//                        printf("\n");
                    }
            ...
            Model::DATA_COLLECTOR->mosquito_recombined_genotype_resistant_count()[loc][16]++;
        }
    }
}
