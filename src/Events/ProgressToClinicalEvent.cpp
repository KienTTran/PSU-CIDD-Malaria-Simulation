...
//   std::cout << p_treatment << std::endl;
  if (p <= p_treatment) {
    ...
    if (person->will_progress_to_death_when_recieve_treatment()) {
      ...
      Model::DATA_COLLECTOR->record_1_malaria_death(person->location(), person->age(),true);
    ...
    if (person->host_state()==Person::DEAD) {
      Model::DATA_COLLECTOR->record_1_malaria_death(person->location(), person->age(),false);
      return;
    }
    //
