          ...
          Model::DATA_COLLECTOR->record_1_mutation_by_drug(person_->location(), blood_parasite->genotype(), new_genotype, drug_id);
            ...
            if (Model::SCHEDULER->current_time() >= Model::CONFIG->start_of_comparison_period()) {
                for(int loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
                    for (int therapy_id = 0; therapy_id < Model::MOSQUITO->therapy_list.size(); therapy_id++) {
                        auto therapy = Model::MOSQUITO->therapy_list[therapy_id];
                        auto resistant_types = Model::MOSQUITO->double_resistant_list[therapy_id].size();
                        for (int resistant_type = 0; resistant_type < resistant_types; resistant_type++) {
                            if (Model::MOSQUITO->genotype_resistant_to(new_genotype, Model::MOSQUITO->double_resistant_list[therapy_id][resistant_type],therapy_id)) {
                                Model::DATA_COLLECTOR->clonal_resistant_genotype_count()[loc][therapy_id][resistant_type]++;
                                Model::DATA_COLLECTOR->monthly_clonal_resistant_genotype_count()[loc][therapy_id][resistant_type]++;
                            }
                        }
                    }
                }
