...
#include <random>
#include <chrono>
#include "RenderEntity.cuh" //Must be before cuda_gl_interop.h
...
GPU::RenderEntity::~RenderEntity() {
}

void GPU::RenderEntity::init_entity(){
...
//        entity_colors_1_pop.push_back(entity_pop_color);//head
    ...
    check_cuda_error(cudaMalloc(&d_ogl_buffer_model_ptr, Model::CONFIG->n_people_init() * Model::CONFIG->gpu_config().pre_allocated_mem_ratio * sizeof(glm::mat4)));
    check_cuda_error(cudaMalloc(&d_ogl_buffer_color_ptr, Model::CONFIG->n_people_init() * Model::CONFIG->gpu_config().pre_allocated_mem_ratio * sizeof(glm::vec4)));
}

void GPU::RenderEntity::init_render_entity(int window_width, int window_height) {
    view = glm::mat4(1.0f);
    ...
    VBO = new GLuint[2];
    SSBO = new GLuint[2];
    shader->load_data(Model::CONFIG->render_config().vertex_shader_path.c_str(),
                      Model::CONFIG->render_config().fragment_shader_path.c_str());
    shader->use();
    ...
    check_cuda_error(cudaGraphicsGLRegisterBuffer(&d_cuda_buffer_model, SSBO[0], cudaGraphicsMapFlagsWriteDiscard));
    check_cuda_error(cudaGraphicsGLRegisterBuffer(&d_cuda_buffer_color, SSBO[1], cudaGraphicsMapFlagsWriteDiscard));
    check_cuda_error(cudaGraphicsMapResources(1, &d_cuda_buffer_model, 0));
    check_cuda_error(cudaGraphicsMapResources(1, &d_cuda_buffer_color, 0));
    check_cuda_error(cudaGraphicsResourceGetMappedPointer((void **)&d_ogl_buffer_model_ptr,
                                                      &d_ogl_buffer_model_num_bytes,
                                                      d_cuda_buffer_model));
    check_cuda_error(cudaGraphicsResourceGetMappedPointer((void **)&d_ogl_buffer_color_ptr,
