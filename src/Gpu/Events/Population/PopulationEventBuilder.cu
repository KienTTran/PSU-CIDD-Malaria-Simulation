...
#include "PopulationEventBuilder.cuh"
...
#include "ChangeTreatmentCoverageEvent.cuh"
...
#include "IntroduceLumefantrineMutantEvent.cuh"
...
#include "SingleRoundMDAEvent.cuh"
#include "TurnOffMutationEvent.cuh"
#include "TurnOnMutationEvent.cuh"
#include "yaml-cpp/yaml.h"

std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_introduce_parasite_events(const YAML::Node& node, Config* config) {
  std::vector<GPU::Event*> events;
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_introduce_parasites_periodically_events(const YAML::Node& node,
                                                                                          Config* config) {
  std::vector<GPU::Event*> events;
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_introduce_parasites_periodically_events_v2(const YAML::Node& node,
                                                                                             Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    const auto location = event_node["location"].as<unsigned long>();
    ...
    for (auto loc = location_from; loc < location_to; ++loc) {
      for (auto j = 0; j < event_node["parasite_info"].size(); j++) {
        ...
        //        allele_distributions(Model::CONFIG->gpu_genotype_info().loci_vector.size());
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_change_treatment_coverage_event(const YAML::Node& node,
                                                                                  Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_change_treatment_strategy_event(const YAML::Node& node,
                                                                                  Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_single_round_mda_event(const YAML::Node& node, Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    const auto starting_date = event_node["day"].as<date::year_month_day>();
    auto time = (date::sys_days { starting_date } - date::sys_days { config->starting_date() }).count();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_modify_nested_mft_strategy_event(const YAML::Node& node,
                                                                                   Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    const auto starting_date = event_node["day"].as<date::year_month_day>();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_turn_on_mutation_event(const YAML::Node& node, Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    const auto starting_date = event_node["day"].as<date::year_month_day>();
    auto time = (date::sys_days { starting_date } - date::sys_days { config->starting_date() }).count();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_turn_off_mutation_event(const YAML::Node& node, Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    const auto starting_date = event_node["day"].as<date::year_month_day>();
    auto time = (date::sys_days { starting_date } - date::sys_days { config->starting_date() }).count();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_introduce_plas2_parasite_events(const YAML::Node& node,
                                                                                  Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    auto location = event_node["location"].as<int>();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_introduce_580Y_mutant_events(const YAML::Node& node, Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    auto location = event_node["location"].as<int>();
    if (location < config->number_of_locations()) {
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_introduce_aq_mutant_parasite_events(const YAML::Node& node,
                                                                                      Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    auto location = event_node["location"].as<int>();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_introduce_lumefantrine_mutant_parasite_events(const YAML::Node& node,
                                                                                                Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    auto location = event_node["location"].as<int>();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_introduce_triple_mutant_to_dpm_parasite_events(const YAML::Node& node,
                                                                                                 Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    auto location = event_node["location"].as<int>();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_change_interrupted_feeding_rate_event(const YAML::Node& node,
                                                                                        Config* config) {
  std::vector<GPU::Event*> events;
  for (const auto& event_node : node) {
    auto location = event_node["location"].as<int>();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_change_within_host_induced_free_recombination_events(const YAML::Node event_node,
                                                                                           Config* config) {
  std::vector<GPU::Event*> events;
  const auto starting_date = event_node["day"].as<date::year_month_day>();
  auto time = (date::sys_days { starting_date } - date::sys_days { config->starting_date() }).count();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build_change_mutation_probability_by_locus_events(const YAML::Node event_node,
                                                                                       Config* config) {
  std::vector<GPU::Event*> events;
  const auto starting_date = event_node["day"].as<date::year_month_day>();
  auto time = (date::sys_days { starting_date } - date::sys_days { config->starting_date() }).count();
...
std::vector<GPU::Event*> GPU::PopulationEventBuilder::build(const YAML::Node& node, Config* config) {
  std::vector<GPU::Event*> events;
  ...
  if (name == "change_mutation_probability_by_locus") {
    events = build_change_mutation_probability_by_locus_events(node["info"], config);
  }
  return events;
}
