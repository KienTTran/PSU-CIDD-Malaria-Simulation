...
GPU::Drug::Drug(GPU::DrugType *drug_type)
    : dosing_days_(0),
      start_time_(0),
      end_time_(0),
...
GPU::Drug::~Drug() = default;

void GPU::Drug::update() {
  const auto current_time = Model::GPU_SCHEDULER->current_time();
  last_update_value_ = get_current_drug_concentration(current_time);
  last_update_time_ = current_time;
}

double GPU::Drug::get_current_drug_concentration(int currentTime) {
...
double GPU::Drug::get_mutation_probability(double currentDrugConcentration) const {
  double P = 0;
...
double GPU::Drug::get_mutation_probability() const {
  return get_mutation_probability(last_update_value_);
}

void GPU::Drug::set_number_of_dosing_days(int dosingDays) {
  dosing_days_ = dosingDays;

  last_update_value_ = 1.0;
  last_update_time_ = Model::GPU_SCHEDULER->current_time();

  start_time_ = last_update_time_;
  end_time_ = last_update_time_ + drug_type_->get_total_duration_of_drug_activity(dosingDays);
}

double GPU::Drug::get_parasite_killing_rate(const int &genotype_id) const {
  return drug_type_->get_parasite_killing_rate_by_concentration(
      last_update_value_, Model::CONFIG->gpu_genotype_db[genotype_id]->get_EC50_power_n(drug_type_));
}
