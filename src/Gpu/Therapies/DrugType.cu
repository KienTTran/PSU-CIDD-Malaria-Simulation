...
GPU::DrugType::~DrugType() = default;

double GPU::DrugType::get_parasite_killing_rate_by_concentration(const double &concentration, const double &EC50_power_n) {
  const auto con_power_n = pow(concentration, n_);
  return maximum_parasite_killing_rate_ * con_power_n / (con_power_n + EC50_power_n);
}

double GPU::DrugType::n() {
  return n_;
}

void GPU::DrugType::set_n(const double &n) {
  n_ = n;
  //    set_EC50_power_n(pow(EC50_, n_));
}

int GPU::DrugType::get_total_duration_of_drug_activity(const int &dosing_days) const {
  // CutOffPercent is 10
  // log2(100.0 / 10.0) = 3.32192809489
  return dosing_days + ceil(drug_half_life_ * LOG2_10);
}

void GPU::DrugType::populate_resistant_aa_locations(Config *config_) {
