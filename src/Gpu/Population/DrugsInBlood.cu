...
GPU::Drug *GPU::DrugsInBlood::add_drug(GPU::Drug *drug) {
  ...
  person_update_info.drug_starting_value[typeID] = drugs_->at(typeID)->starting_value();
  person_update_info.drug_dosing_days[typeID] = drugs_->at(typeID)->dosing_days();
  person_update_info.drug_last_update_value[typeID] = drugs_->at(typeID)->last_update_value();
  person_update_info.drug_last_update_time[typeID] = drugs_->at(typeID)->last_update_time();
  person_update_info.drug_start_time[typeID] = drugs_->at(typeID)->start_time();
  person_update_info.drug_end_time[typeID] = drugs_->at(typeID)->end_time();
  person_update_info.drug_half_life[typeID] = drugs_->at(typeID)->drug_type()->drug_half_life();
  person_update_info.drug_rand_uniform_1[typeID] = 0.1;//Model::RANDOM->random_uniform_double(-0.2, 0.2);
  person_update_info.drug_rand_uniform_2[typeID] = 0.1;//Model::RANDOM->random_uniform_double(0, 0.1);
  int person_current_drug_id = person_update_info.drug_in_blood_type_id_current_index;
  person_update_info.drug_in_blood_type_id[person_current_drug_id] = typeID;
  person_update_info.drug_in_blood_type_id_current_index += 1;
  person_update_info.drug_in_blood_size = drugs_->size();
  person_update_info.person_has_drug_in_blood = true;
