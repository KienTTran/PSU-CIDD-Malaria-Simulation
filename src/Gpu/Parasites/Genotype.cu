...
GPU::Genotype *GPU::Genotype::combine_mutation_to(const int &locus, const int &value) {
  // TODO: remove this
  return this;
}
...
std::ostream &operator<<(std::ostream &os, const GPU::Genotype &e) {
  os << e.genotype_id << "\t";
  os << e.get_aa_sequence();
  return os;
}

std::string GPU::Genotype::get_aa_sequence() const {
  return aa_sequence;
}
bool GPU::Genotype::is_valid(const PfGeneInfo &gene_info) {
...
bool GPU::Genotype::match_pattern(const std::string &pattern) {
  int id = 0;
...
std::string GPU::Genotype::Convert_PfGenotypeStr_To_String(const PfGenotypeStr &pfGenotypeStr) {
  std::stringstream ss;
...
GPU::Genotype *GPU::Genotype::free_recombine(Config *config, ::Random *pRandom, GPU::Genotype *f, GPU::Genotype *m) {
  ...
  auto new_aa_sequence = Convert_PfGenotypeStr_To_String(new_pf_genotype_str);
  return config->gpu_genotype_db.get_genotype(new_aa_sequence, config);
}
