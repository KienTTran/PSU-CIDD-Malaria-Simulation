...
GPU::NovelDrugIntroductionStrategy::NovelDrugIntroductionStrategy() {
  name = "NovelDrugIntroductionStrategy";
  type = NovelDrugIntroduction;
}


std::string GPU::NovelDrugIntroductionStrategy::to_string() const {
...
void GPU::NovelDrugIntroductionStrategy::monthly_update() {
  ...
  if (!is_switched) {
    ...
    if (Model::GPU_SCHEDULER->current_time() > 3000 && Model::GPU_DATA_COLLECTOR->current_tf_by_therapy()[current_public_therapy_id] >= tf_threshold) {

      // switch to novel drugs

      auto novel_SFT_strategy = Model::CONFIG->gpu_strategy_db()[newly_introduced_strategy_id];
      ...
      new_public_stategy->starting_time = Model::GPU_SCHEDULER->current_time();

      strategy_list[0] = new_public_stategy;
      new_public_stategy->id = Model::CONFIG->gpu_strategy_db().size();

      Model::CONFIG->gpu_strategy_db().push_back(new_public_stategy);

      //reset the time point to collect ntf
      Model::CONFIG->start_of_comparison_period() = Model::GPU_SCHEDULER->current_time();

      //reset the total time to 10 years after this time point
      const date::sys_days next_10_year{date::year_month_day{Model::GPU_SCHEDULER->calendar_date} + date::years{10}};
      const auto new_total_time = Model::GPU_SCHEDULER->current_time() + TimeHelpers::number_of_days(
          Model::GPU_SCHEDULER->calendar_date, next_10_year
      );

      if (new_total_time > Model::CONFIG->total_time()) {
        //extend the scheduler
        Model::GPU_SCHEDULER->extend_total_time(new_total_time);
      }
      Model::CONFIG->total_time() = new_total_time;

      LOG(INFO) << date::year_month_day{Model::GPU_SCHEDULER->calendar_date} << ": Switch to novel drug with id "
      ...
      if (new_total_time > Model::CONFIG->total_time()) {
        //extend the scheduler
        Model::GPU_SCHEDULER->extend_total_time(new_total_time + 10 * 365);
