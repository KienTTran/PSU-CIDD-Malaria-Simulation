...
GPU::MFTMultiLocationStrategy::MFTMultiLocationStrategy() : GPU::IStrategy(
...
std::string GPU::MFTMultiLocationStrategy::to_string() const {
...
void GPU::MFTMultiLocationStrategy::update_end_of_time_step() {
  //do nothing here
}

void GPU::MFTMultiLocationStrategy::adjust_started_time_point(const int &current_time) {
  starting_time = current_time;
}

void GPU::MFTMultiLocationStrategy::monthly_update() {
  if (peak_after==-1) {
    ...
    // increasing linearly
    if (Model::GPU_SCHEDULER->current_time() <= starting_time + peak_after) {
      if (distribution[0][0] < 1) {
        for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
          for (auto i = 0; i < distribution[loc].size(); i++) {
            const auto dist = (peak_distribution[loc][i] - start_distribution[loc][i])*
                (Model::GPU_SCHEDULER->current_time() - starting_time)
                /peak_after +
                start_distribution[
                    loc][i];
