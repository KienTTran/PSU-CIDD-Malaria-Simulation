...
#include <cmath>
#include <algorithm>
#include "ModelDataCollector.cuh"
#include "Model.h"
...
GPU::ModelDataCollector::ModelDataCollector(Model* model) : model_(model), current_utl_duration_(0),
...
GPU::ModelDataCollector::~ModelDataCollector()
= default;

void GPU::ModelDataCollector::initialize() {
  if (model_ != nullptr) {
    ...
    popsize_by_location_hoststate_ = IntVector2(
        Model::CONFIG->number_of_locations(),
        IntVector(GPU::Person::NUMBER_OF_STATE, 0));
    ...
    popsize_by_location_age_class_by_5_ = IntVector2(
      ...
      popsize_by_location_hoststate_age_class_ = IntVector3(
              Model::CONFIG->number_of_locations(),
              IntVector2(GPU::Person::NUMBER_OF_STATE, IntVector(Model::CONFIG->number_of_age_classes(), 0)));
...
void GPU::ModelDataCollector::perform_population_statistic() {
  ...
  //    popsize_by_location_hoststate_.assign(Model::CONFIG->number_of_locations(), IntVector(GPU::Person::NUMBER_OF_STATE, 0));
  //
  ...
  for (auto location = 0ul; location < Model::CONFIG->number_of_locations(); location++) {
    ...
    for (auto i = 0; i < GPU::Person::NUMBER_OF_STATE; i++) {
  ...
  for (auto loc = 0ul; loc < Model::CONFIG->number_of_locations(); loc++) {
    ...
    for (auto hs = 0; hs < GPU::Person::NUMBER_OF_STATE - 1; hs++) {
      for (auto ac = 0ul; ac < Model::CONFIG->number_of_age_classes(); ac++) {
        ...
        for (int i = 0; i < size; i++) {
          GPU::Person* p = pi->vPerson()[loc][hs][ac][i];
          ...
          } else if (hs == GPU::Person::CLINICAL) {
            number_of_positive_by_location_[loc]++;
            number_of_positive_by_location_age_group_[loc][ac] += 1;
            blood_slide_prevalence_by_location_[loc] += 1;
            blood_slide_number_by_location_age_group_[loc][ac] += 1;
    ...
    last_10_blood_slide_prevalence_by_location_[loc][
        (Model::GPU_SCHEDULER->current_time() / Model::CONFIG->report_frequency()) %
...
void GPU::ModelDataCollector::collect_number_of_bites(const int& location, const int& number_of_bites) {
...
void GPU::ModelDataCollector::perform_yearly_update() {
  if (Model::GPU_SCHEDULER->current_time() == Model::CONFIG->start_collect_data_day()) {
    for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
      person_days_by_location_year_[loc] = Model::GPU_POPULATION->size(loc) * Constants::DAYS_IN_YEAR();
    }
  } else if (Model::GPU_SCHEDULER->current_time() > Model::CONFIG->start_collect_data_day()) {
    for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
...
void GPU::ModelDataCollector::update_person_days_by_years(const int& location, const int& days) {
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
    person_days_by_location_year_[location] += days;
  }
}

void GPU::ModelDataCollector::calculate_eir() {
  for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    if (EIR_by_location_year_[loc].empty()) {
      //collect data for less than 1 year
      const auto total_time_in_years =
          (Model::GPU_SCHEDULER->current_time() - Model::CONFIG->start_collect_data_day()) /
...
void GPU::ModelDataCollector::collect_1_clinical_episode(const int& location, const int& age, const int& age_class) {
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
...
void GPU::ModelDataCollector::record_1_death(
    const int& location, const int& birthday, const int& number_of_times_bitten,
    const int& age_group, const int& age
) {
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
    update_person_days_by_years(location, -(Constants::DAYS_IN_YEAR() - Model::GPU_SCHEDULER->current_day_in_year()));
...
void GPU::ModelDataCollector::record_1_malaria_death(const int& location, const int& age) {
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
    if (age < 79) {
...
void GPU::ModelDataCollector::update_average_number_bitten(
...
void GPU::ModelDataCollector::calculate_percentage_bites_on_top_20() {
  auto pi = Model::GPU_POPULATION->get_person_index<GPU::PersonIndexByLocationStateAgeClass>();
  for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    for (auto hs = 0; hs < GPU::Person::NUMBER_OF_STATE - 1; hs++) {
...
void GPU::ModelDataCollector::record_1_non_treated_case(const int& location, const int& age) {
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
    if (age <= 79) {
...
void GPU::ModelDataCollector::begin_time_step() {
...
//TODO: review
void GPU::ModelDataCollector::end_of_time_step() {
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
    double avg_tf = 0;
    for (auto location = 0; location < Model::CONFIG->number_of_locations(); location++) {
      total_number_of_treatments_60_by_location_[location][Model::GPU_SCHEDULER->current_time() %
                                                           Model::CONFIG->tf_window_size()] = today_number_of_treatments_by_location_[location];
      total_RITF_60_by_location_[location][Model::GPU_SCHEDULER->current_time() %
                                           Model::CONFIG->tf_window_size()] = today_RITF_by_location_[location];
      total_TF_60_by_location_[location][Model::GPU_SCHEDULER->current_time() %
...
void GPU::ModelDataCollector::record_1_treatment(const int& location, const int& age, const int& therapy_id) {
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
...
void GPU::ModelDataCollector::record_1_mutation(const int& location, Genotype* from, Genotype* to) {
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
...
void GPU::ModelDataCollector::update_UTL_vector() {
  UTL_duration_.push_back(current_utl_duration_);
  current_utl_duration_ = 0;
}

//void GPU::ModelDataCollector::collect_1_non_resistant_treatment(const int& therapy_id) {
//    number_of_treatments_with_therapy_ID_[therapy_id] += 1;
//}

void GPU::ModelDataCollector::record_1_TF(const int& location, const bool& by_drug) {
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
  ...
  if (Model::GPU_SCHEDULER->current_time() >= Model::CONFIG->start_of_comparison_period()) {
    const auto current_discounted_tf = exp(
        log(0.97) * floor((Model::GPU_SCHEDULER->current_time() - Model::CONFIG->start_collect_data_day()) /
                          Constants::DAYS_IN_YEAR()));
...
void
GPU::ModelDataCollector::record_1_treatment_failure_by_therapy(const int& location, const int& age, const int& therapy_id) {
  number_of_treatments_fail_with_therapy_ID_[therapy_id] += 1;
  today_tf_by_therapy_[therapy_id] += 1;
}

void GPU::ModelDataCollector::record_1_treatment_success_by_therapy(const int& therapy_id) {
  number_of_treatments_success_with_therapy_ID_[therapy_id] += 1;
}

void GPU::ModelDataCollector::update_after_run() {
...
void GPU::ModelDataCollector::record_AMU_AFU(
    ...
    if (sc_therapy != nullptr) {
      ...
      if (art_id != -1 && sc_therapy->drug_ids.size() > 1) {
        ...
        const auto discounted_fraction = exp(
            log(0.97) *
            floor((Model::GPU_SCHEDULER->current_time() - Model::CONFIG->start_of_comparison_period()) /
                  Constants::DAYS_IN_YEAR()));
        //            assert(false);
        //combine therapy
        for (auto i = 0; i < number_of_drugs_in_therapy; i++) {
          ...
          if (drug_id != art_id) {
            ...
            for (auto j = 0ul; j < parasite_population_size; j++) {
              GPU::ClonalParasitePopulation* bp = person->all_clonal_parasite_populations()->parasites()->at(j);
...
double GPU::ModelDataCollector::get_blood_slide_prevalence(const int& location, const int& age_from, const int& age_to) {
...
void GPU::ModelDataCollector::monthly_update() {
  if (Model::GPU_SCHEDULER->current_time() > Model::CONFIG->start_collect_data_day()) {
...
void GPU::ModelDataCollector::record_1_migration(GPU::Person* pPerson, const int& from, const int& to) {

}

void GPU::ModelDataCollector::check_nan_inf(double &value){
    if(std::isnan(value)){
        value = 0;
    }
    if(std::isinf(value)){
