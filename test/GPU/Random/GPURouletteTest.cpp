...
TEST_F(GPURouletteTest, Sampling_with_sum_distribution_0) {
    ...
    auto results = gr.roulette_sampling<Person>(n_location, n_sample, d_distribution, all_person, d_sum_distribution,false);
...
TEST_F(GPURouletteTest, Sampling_with_no_sum_distribution) {
    ...
    for (int n = 0; n < n_repeat; ++n) {
        auto results = gr.roulette_sampling<Person>(n_location, n_sample, d_distribution, all_person, d_sum_distribution_neg,false);
...
TEST_F(GPURouletteTest, Sampling_with_2_in_all) {
    ...
    ThrustTVectorDevice<double> d_sum_distribution = sum_distribution;
    for (int n = 0; n < n_repeat; ++n) {
        auto results = gr.roulette_sampling<Person>(n_location, n_sample, d_distribution, all_person,
