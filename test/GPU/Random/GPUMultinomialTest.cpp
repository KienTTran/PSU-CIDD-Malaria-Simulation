...
TEST_F(GPUMultinomialTest, Sampling_with_sum_distribution_0) {
...
TEST_F(GPUMultinomialTest, Sampling_with_no_sum_distribution) {
...
TEST_F(GPUMultinomialTest, Sampling_with_one_in_all) {
    distribution = std::vector<double>(n_location*n_person, 0.0);
    for(int loc = 0; loc < n_location; loc++) {
...
TEST_F(GPUMultinomialTest, Sampling_with_2_in_all) {
    distribution = std::vector<double> (n_location *n_person,0.0);
    for (int loc = 0; loc < n_location; loc++) {
...
TEST_F(GPUMultinomialTest, Sampling_with_4_in_all) {
    distribution = std::vector<double>(n_location*n_person, 0.0);
    for(int loc = 0; loc < n_location; loc++) {
...
TEST_F(GPUMultinomialTest, compare_with_roulette) {
    distribution = std::vector<double>(n_location*n_person, 0.0);
    for(int loc = 0; loc < n_location; loc++) {
        for (int i = 0; i < n_person; ++i) {
            distribution[loc*n_person+i] = r.random_uniform();
